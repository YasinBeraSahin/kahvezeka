...
log_file = open("init_log_v5.txt", "w", encoding="utf-8")
def log(msg):
    print(msg)
...
try:
    ...
    def get_password_hash(password):
        ...
        # Use a cost of 12 (default) or less if slow, but generic is fine.
    ...
    if not business:
        ...
        business = models.Business(
            name=detailed_name,
            # Removed 'description' field as it does not exist in models.py
            address="Caferağa Mah. Moda Cad. No: 12, Kadıköy/İstanbul",
            latitude=40.9882,
            longitude=29.0223,
            owner_id=mekan_user.id,
            is_approved=True,
            # average_rating and review_count are computed properties, cannot set them directly if using strict model?
            # NO, they are hybrid properties. But in SQLAlchemy models, if they are @hybrid_property, 
            # we usually can't set them in constructor unless they map to a column.
            # in models.py:
            # @hybrid_property def average_rating(self): ...
            # It is NOT a column. It is calculated from reviews.
            # So passing average_rating=4.8 to constructor will FAIL too!
